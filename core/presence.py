...
def _locate_model(name):
    ...
    与两个 .onnx 模型文件（与 main.py / 打包后的 exe 同级）保持一致。
    """
...
class PresenceDetector:
    ...
    def reset_last_seen(self):
        ...
        供进入休息阶段时调用：休息无人关遮罩的宽限期应从休息开始起算，
        而不是沿用工作段最后一次看到人的时刻（否则用户离开较久后进入
        休息，遮罩可能刚显示就被判定"休息无人超时"而关闭）。
        """
        with self._lock:
    ...
    def _run(self):
        ...
        if not self._acquire():
            # 摄像头可能已打开但模型加载失败（_acquire 返回 False 时
            # _cap 已赋值）——必须释放摄像头句柄，否则灯常亮/被占用
            self._release()
            with self._lock:
                self._available = False
